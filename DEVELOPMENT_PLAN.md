...
- **数据层**（`src/state/`）：`RulerState` 接口定义，所有状态变更通过纯函数 `produceState(currentState, action): RulerState` 完成。M1 先实现基础状态（变换参数、参考线集合），不可变快照与历史栈在 M4 完善。
...
**验收标准**：
- 面板操作支持撤销（undo 后参考线状态恢复）。
- WCAG 2.1 AA 对比度达标（所有文本/背景组合 ≥ 4.5:1）。
- 屏幕阅读器（NVDA/VoiceOver）可正确朗读参考线位置与状态。

---

### Milestone 4：平台级功能扩展（预计 5 周）

**目标**：多画布管理、历史记录/协作基础、缩略图导航、插件系统。

#### W15：不可变状态快照 + 历史栈

**技术方案**：
- **不可变状态**：
  - `RulerState` 接口包含：变换参数、参考线集合、用户配置、历史操作记录。
  - 所有变更通过纯函数 `produceState(currentState, action): RulerState` 完成。
  - 内部采用结构共享：仅复制变更路径上的对象，共享未变更引用，内存开销 O(log n)。
- **双栈设计**：
  - `undoStack` + `redoStack`
  - 操作时将当前状态快照压入 `undoStack`
  - 撤销时弹出并压入 `redoStack`
  - `undo()` / `redo()` 为 O(1) 操作
- **内存策略**：
  - `unlimited`：无限制（小型项目）
  - `count-limited`：限制栈深度（默认 50 步）
  - `memory-limited`：限制总内存占用，基于快照大小估算
- **JSON Patch 序列化**（RFC 6902）：记录增量变更而非完整快照，体积 < 500B/典型操作。
- **分组操作**：支持将多个连续操作合并为单一历史记录项（如"创建 10 条参考线"作为一次操作）。

**验收标准**：
- 50 步历史，undo/redo 操作耗时 O(1)（< 1ms）。
- JSON Patch 序列化体积 < 500B（创建/删除/移动参考线三种典型操作各测 10 次取平均）。
...
**风险点**：多画布 + 历史记录 + 插件的组合可能导致状态管理复杂度爆炸。  
**缓解措施**：W15 建立"状态不变量"自动化测试（如"undo 到任意点后 redo 应回到最新"）；W18 前冻结核心状态接口，插件仅通过钩子只读访问。

---

#### W19：工具栏插槽化 + 跨画布复制粘贴 + 页面模板
