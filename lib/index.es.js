...
/*!
* vue3-sketch-ruler v1.2.1
* 2021年10月Sun Oct 31 2021 16:53:30 GMT+0800 (中国标准时间)
* 制作
*/
...
const drawCavaseRuler = (ctx, start, selectStart, selectLength, options, h) => {
  ...
  const { bgColor, fontColor, shadowColor, ratio, longfgColor, shortfgColor } = palette;
  ...
  setup(props, { emit }) {
    ...
    const drawRuler = () => {
      ...
      if (props.vertical && state.canvasContext) {
        drawCavaseRuler(state.canvasContext, props.start, props.selectStart, props.selectLength, options);
      } else if (state.canvasContext) {
        drawCavaseRuler(state.canvasContext, props.start, props.selectStart, props.selectLength, options, true);
      }
    };
