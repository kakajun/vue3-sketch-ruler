...
1. package.json vue3-sketch-ruler 版本从 ^1.3.3 升级到 ^2.4.0（已改好）
2. src/plugins/customComponents.ts旧版：import { SketchRule } from 'vue3-sketch-ruler'（命名导出）新版：import SketchRule from 'vue3-sketch-ruler'（默认导出）
3. src/views/chart/ContentEdit/components/EditRule/index.vue 主要适配 2.x 的 API 变化：
...
移除了 startX / startY 的手动计算和 handleScroll scale 改用 computed 的 getter/setter 配合 v-model:scale 添加了 :shadow 属性传入画布尺寸拖拽逻辑改为通过 panzoomInstance.pan() 控制 4. src/main.ts 样式路径 vue3-sketch-ruler/lib/style.css 保持不变，2.x 依然兼容
