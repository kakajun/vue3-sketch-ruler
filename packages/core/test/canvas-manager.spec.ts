...
describe('CanvasManager', () => { // 画布管理器测试
  ...
  it('should initialize empty', () => { // 初始化为空状态
    expect(manager.getState().canvases).toEqual([])
    expect(manager.activeCanvas).toBeNull()
  })

  it('should add canvas and auto-activate first', () => { // 添加画布并自动激活第一个
  ...
  it('should notify on update', () => { // 更新时发出通知
    let notified = false
    manager.onUpdate(() => {
      notified = true
    })
  ...
  it('should remove canvas and switch to another', () => { // 移除画布并自动切换到另一个
    const id1 = manager.addCanvas({ name: 'c1', width: 800, height: 600 })
    const id2 = manager.addCanvas({ name: 'c2', width: 1024, height: 768 })
    manager.switchCanvas(id2)
  ...
  it('should update canvas state', () => { // 更新画布状态
  ...
  it('should merge global lines with local lines', () => { // 合并全局参考线与本地参考线
  ...
  it('should export and import canvas snapshot', () => { // 导出和导入画布快照
    const id = manager.addCanvas({ name: 'c1', width: 800, height: 600 })
  ...
  it('should register and apply custom template', () => { // 注册并应用自定义模板
  ...
  it('should have builtin templates', () => { // 拥有内置模板
...
describe('BUILTIN_TEMPLATES', () => { // 内置模板测试
  it('should define all expected templates', () => { // 定义所有预期的模板
