...
const initStart = () => {
  ...
  // 可用宽度和高度，减去两边的内边距
  const availableWidth = (props.width - props.thick) * (1 - props.paddingRatio)
  const availableHeight = (props.height - props.thick) * (1 - props.paddingRatio)
  ...
  if (props.canvasWidth > props.width || props.canvasHeight > props.height) {
    ...
    zoomStartY.value = (props.height - props.canvasHeight * scale) / 2
    zoomStartX.value = (props.width - props.canvasWidth * scale) / 2
  }

  // }

  console.log(scale, 'scale')
  console.log(scaleWidth, scaleHeight, 'scaleHeight')
  console.log(props.width * props.paddingRatio, 'props.paddingRatio')
